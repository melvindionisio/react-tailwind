...
const App = () => {
  ...
  const addTask = (task) => {
    ...
    // setTimeout(() => {
    //   webpad.current.classList.contains("fade-in") &&
    //     webpad.current.classList.remove("fade-in");
    // }, 550);

    const refTask = taskRef.current;
    refTask.classList.add("bubble");
    setTimeout(() => {
      refTask.classList.contains("bubble") &&
        refTask.classList.remove("bubble");
    }, 500);
  };
  ...
  const deleteTask = (id) => {
    /*  const refTask = taskRef.current;
    refTask.classList.add("scale-in-out");
    setTimeout(() => {
      refTask.classList.contains("scale-in-out") &&
        refTask.classList.remove("scale-in-out");
    }, 500); */
  ...
  return (
    <div className="grid items-start min-h-screen p-4 text-gray-800 bg-gray-900 select-none place-items-center">
      <div
        className="w-full transition-all duration-500 bg-gray-800 shadow-lg rounded-xl rounded-t-2xl md:w-9/12 lg:w-6/12"
        ref={webpad}
      >
        <Header
